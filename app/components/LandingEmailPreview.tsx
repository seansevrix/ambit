...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center rounded-full border border-black/10 bg-white/90 px-3 py-1 text-[11px] font-semibold text-black/60 shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
      {children}
    </span>
  );
}
...
function ScorePill({ score }: { score: number }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/95 px-2.5 py-1 text-[11px] font-semibold text-black/70 shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
...
function ReasonChip({ text }: { text: string }) {
  return (
    <span className="inline-flex items-center rounded-full border border-black/10 bg-white/95 px-3 py-1 text-[11px] font-semibold text-black/60">
      {text}
    </span>
  );
}
...
function EmailMatchCard({ m }: { m: PreviewMatch }) {
  return (
    <div className="rounded-3xl bg-white/85 backdrop-blur-md shadow-[0_30px_90px_rgba(0,0,0,0.12)]">
      <div className="px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="truncate text-[15px] font-black tracking-tight text-black">{m.title}</div>
            ...
            <div className="mt-2 text-xs text-black/45">Posted {m.posted}</div>
          </div>

          <div className="shrink-0 flex items-center gap-2">
            <button
              type="button"
              className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-black/45 shadow-[0_12px_35px_rgba(0,0,0,0.10)]"
              aria-label="Save"
              title="Save"
            >
              ☆
            ...
            <button
              type="button"
              className="inline-flex items-center rounded-full bg-white/95 px-3 py-1 text-[11px] font-semibold text-black/70 shadow-[0_12px_35px_rgba(0,0,0,0.10)]"
        ...
        <div className="mt-4 rounded-2xl bg-white/80 p-4 shadow-[0_12px_35px_rgba(0,0,0,0.07)]">
...
export default function LandingEmailPreview() {
  return (
    <div className="w-full">
      {/* This is now FLOATING content (no outer border/panel). */}
      <div className="flex flex-col gap-4">
        {/* Context bar floats too */}
        ...
        {/* Grid floats on the landing background */}
        <div className="grid items-start gap-8 lg:grid-cols-[1.35fr_0.85fr]">
          ...
          {/* RIGHT */}
          <aside className="rounded-3xl bg-white/85 backdrop-blur-md p-7 shadow-[0_30px_90px_rgba(0,0,0,0.12)]">
            ...
            <div className="mt-7 rounded-2xl bg-white/90 p-5 shadow-[0_18px_55px_rgba(0,0,0,0.08)]">
